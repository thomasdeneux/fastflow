...
for j=1:length(jitter)
    ...
    for t = 1:nt %loop over time
        ...
        vec = ones(nj,2*gausslat+1);
        speed=zeros(nj,2*gausslat+1);
        for i = 1:max(size(indices)) % loop over all RBCs
            if (t<begchange)
                vec(:,:)=vec(:,:) + exp( -( ( [1:nj]' + (t-1)*(step) - indices1(i) )/sigma  ).^2  )*h;
                %speed = vec * ((t-1)*(step)-indices1(i));
                if t > 1
                    dummy=sum(sum(exp( -( ( [1:nj]' + (t-1)*(step) - indices1(i) )/sigma  ).^2  )*h));
                    %speed = speed+((rr(i)+step)*exp( -( ( [1:nj]' + (t-1)*(step) - indices1(i) )/sigma  ).^2  )*h)/dummy;
                    speed = speed+((rr(i)+step)*exp( -( ( [1:nj]' + (t-1)*(step) - indices1(i) )/sigma  ).^2  )*h);
                end
            elseif (t>begchange-1)&(t<endchange+1)
                vec(:,:)=vec(:,:)+exp(-(([1:nj]'+((begchange-2)*step+(t-(begchange-1))*(step+change))-indices1(i))/sigma).^2)*h;
                if t > 1
                    dummy=sum(sum(exp(-(([1:nj]'+((begchange-2)*step+(t-(begchange-1))*(step+change))-indices1(i))/sigma).^2)*h));
                    %speed = speed+((rr(i)+step+change)*exp(-(([1:nj]'+((begchange-2)*step+(t-(begchange-1))*(step+change))-indices1(i))/sigma).^2)*h)/dummy;
                    speed = speed+((rr(i)+step+change)*exp(-(([1:nj]'+((begchange-2)*step+(t-(begchange-1))*(step+change))-indices1(i))/sigma).^2)*h);

                end
            else
                vec(:,:)=vec(:,:)+exp(-(([1:nj]'+((begchange-2)*step+(endchange-begchange)*(step+change)+(t-(endchange-1))*step)-indices1(i))/sigma).^2)*h;
                if t > 1
                    dummy=sum(sum(exp(-(([1:nj]'+((begchange-2)*step+(endchange-begchange)*(step+change)+(t-(endchange-1))*step)-indices1(i))/sigma).^2)*h));
                    %speed = speed+((rr(i)+step)*exp(-(([1:nj]'+((begchange-2)*step+(endchange-begchange)*(step+change)+(t-(endchange-1))*step)-indices1(i))/sigma).^2)*h)/dummy;
                    speed = speed+((rr(i)+step)*exp(-(([1:nj]'+((begchange-2)*step+(endchange-begchange)*(step+change)+(t-(endchange-1))*step)-indices1(i))/sigma).^2)*h)/dummy;
                end
            end
