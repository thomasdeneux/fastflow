% [timeaxis,conditions,multiplicity] = readconditionLD3(infile,stimlist,flag);
% 
...
function [timeaxis,conditions,multiplicity]=readconditionLD3(infile,stimlist,flag);
...
if ischar(infile)
    s = readconditionfile(infile,flag);
else
    s = infile;
end
% check for LDind columns only
if size(s,2)~=max(LDind)
    error('data should have LDind columns : time - flags - digital square - laser doppler (columns 1,2,6,7 from REX file)')
end
...
for j = 1:size(trialbegindnew,1)
    dummy=find(ismember(s(trialbegindnew(j):wincloseind(j),2),stimlist));
...
% sdata contains the parts of s where data is not zero
sdataind = find(s(:,LDind(1)));
...
for i=1:nstim
    ...
    if isempty(f), error('no data found at time point'), end
    stimbeginddata(i) = f;
end
...
datapointsaftstimmin=min(s(wincloseind,1)-s(stimbegindnew,1));
...
for i = 1:length(trnum)
    timeaxis(:,i)=timeaxis(:,i)-timeaxis(datapointsbefstimmin+1,i) - 500; % centering the timeaxis on (stimbegind + 500)
end
clear snew;
...
for i=1:length(stimIDind)
    ...
    hp(f) = plot(s(stimIDind(i),1)/1000,400,DEC(f,:));
end
try
