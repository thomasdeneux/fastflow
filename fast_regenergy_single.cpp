...
// all computations are done in single
// matrices input/output is in single
// but vecteur input/output is still in double!

class vecteur {
  int n;
  float* t;
public:
  vecteur(int N) {n=N; t=new float[n];}
  vecteur(const vecteur& v) {n=v.n; t=new float[n]; for (int i=0;i<n;i++) t[i]=v.t[i];}
  ~vecteur() {delete [] t;}
  const vecteur& operator=(const vecteur& v) {
    delete [] t; n=v.n; t=new float[n]; 
...
class matrice {
  ...
  bool mxfree;
public:
  matrice(int M, int N) {m=M; n=N; mxfree=true; t=new float[m*n];}
  matrice(const matrice& v) {
    m=v.m; n=v.n; mxfree=true; t=new float[m*n]; for (int i=0;i<m*n;i++) t[i]=v.t[i];}
  matrice(const mxArray* A) {
    if (!mxIsClass(A,"single") || mxIsComplex(A))
      mexErrMsgTxt("mxArray2matrice (float): input array must be of class single");
  ...
  const matrice& operator=(const matrice& v) {
    delete [] t; m=v.m; n=v.n; t=new float[m*n]; 
  ...
  inline float* pointer() const {return t;}
  inline float*& pointer() {return t;}
...
vecteur mxArray2vecteur(const mxArray* A){
    if (!mxIsClass(A,"double") || mxIsComplex(A))
      mexErrMsgTxt("mxArray2vecteur: input array must be of class double");
	...
	vecteur a(n); 
	for (int i=0; i<n; i++) a(i)=(float)p[i];
	return a;
}

matrice mxArray2matrice(const mxArray* A){
    if (!mxIsClass(A,"single") || mxIsComplex(A))
      mexErrMsgTxt("mxArray2matrice: input array must be of class single");
...
mxArray* vecteur2mxArray(const vecteur& x){
    ...
    dims[0] = x.size();
    dims[1] = (x.size())?1:0;
    mxArray* B = mxCreateNumericArray(2,dims,mxDOUBLE_CLASS,mxREAL);
	double* pr = (double*)mxGetData(B);
	for (unsigned int i=0; i<x.size(); i++) pr[i] = (double)x(i);
	return B;
}
...
mxArray* matrice2mxArray(const matrice& x){
    ...
    dims[0] = x.nlin();
    dims[1] = x.ncol();
    mxArray* B = mxCreateNumericArray(2,dims,mxSINGLE_CLASS,mxREAL);
	float* pr = (float*)mxGetData(B);
...
void floormodulo(float u, int& p, float& x) {
	p = (int) u;
	x = u-p;
}
		  ...
		  int nargin, const mxArray* pargin[] )    
{
	...
	for (int k=0; k<nind; k++) {
	  ...
	  float u=coordsM(0,k)-1, v=coordsM(1,k)-1;
	  int p,q; float x,y;
	  floormodulo(u,p,x); floormodulo(v,q,y);
	  // linear interpolation
	  float CS2invji;
