...
%% 0 - few initialization
set(0,'defaultfigurecolormap',gray(64))
fn_imvalue image

%% 1 - new project or continue existing one

if ~exist('S','var') || ~isfield(S,'step')
    if exist('S','var')
        answer = questdlg('overwrite variable ''S''?','','Yes','No','No');
        if ~strcmp(answer,'Yes'), disp('batch interrupted'), return, end
    end
    answer = questdlg('Start new analysis or continue existing one?','fast_mainbatch.m','New','Open','New');
    switch answer
        case 'New'
            S = struct('step','');
        case 'Open'
